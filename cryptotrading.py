...
def trade(prices, stock, sens, settings_code):
    ...
    buy, sell = model(prices, sens = sens, volumes = volumes, volatilities = volatilities, rsis = rsis, first = not bool(stock.traded), stock = stock)
...
def auto_trade(stocks):
    ...
    while True:
        for stock in stock_objects:
            ...
            print("SETTINGS CODE:", settings_code)
...
def signal(stock, new_signal, buys, sells):
    history = plt.subplot()
    history.title.set_text(dt.datetime.now())
    history.scatter(range(len(prices[-50:])), buys[-50:], c = "green")
    history.scatter(range(len(prices[-50:])), sells[-50:], c = "red")
    history.plot(prices[-50:])
    history.set_ylim(np.min(prices[-50:]) - 5, np.max(prices[-50:]) + 5)
